...
void f5()
{
	...
	if(mas[1] == mas[4] and mas[2] == mas[3])//почему-то косяк. число 1001 не проходит, заменить на побитовое сравнение, может поможет.
	{
...
void f6()
{
	...
	while(not(A != B and A + B > C and C > 0))//проверка свойств прямоугольного треугольника, доделать, вроде некоторые проходят.
	{
		//пока пусто тут.
	}
